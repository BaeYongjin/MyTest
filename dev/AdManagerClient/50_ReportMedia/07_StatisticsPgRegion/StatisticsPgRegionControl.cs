// ===============================================================================
//
// StatisticsPgRegionControl.cs
//
// 컨텐츠리포트 지역별통계 컨트롤을 정의합니다. 
//
// ===============================================================================
// Release history
// ===============================================================================
...
namespace AdManagerClient
{
    ...
    /// 컨텐츠리포트 지역별통계 관리 컨트롤
    /// </summary>
    public class StatisticsPgRegionControl : System.Windows.Forms.UserControl, IUserControl
    {
        #region 이벤트핸들러
        ...
        // 사용할 정보모델
        StatisticsPgRegionModel statisticsPgRegionModel  = new StatisticsPgRegionModel();	// 프로그램별 광고시청집계모델
		...
		private AdManagerClient._50_ReportMedia.ReportMediaDs statisticsPgRegionDs;
        private System.ComponentModel.IContainer components;

        public StatisticsPgRegionControl()
        {
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(StatisticsPgRegionControl));
            ...
            this.statisticsPgRegionDs = new AdManagerClient._50_ReportMedia.ReportMediaDs();
            ...
            ((System.ComponentModel.ISupportInitialize)(this.statisticsPgRegionDs)).BeginInit();
            ...
            // dvReport
            // 
            this.dvReport.Table = this.statisticsPgRegionDs.ReportRegion;
            // 
            // statisticsPgRegionDs
            // 
            this.statisticsPgRegionDs.DataSetName = "ReportMediaDs";
            this.statisticsPgRegionDs.Locale = new System.Globalization.CultureInfo("en-US");
            this.statisticsPgRegionDs.SchemaSerializationMode = System.Data.SchemaSerializationMode.IncludeSchema;
            // 
            // grdExRepoert
            // 
            ...
            // 
            // StatisticsPgRegionControl
            // 
            this.Controls.Add(this.uiPanelChoiceAdSchedule);
            this.Name = "StatisticsPgRegionControl";
            ...
            ((System.ComponentModel.ISupportInitialize)(this.statisticsPgRegionDs)).EndInit();
...
//			}
		...
		private void Init_MediaCode()
		{
			...
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgRegionDs.Media, mediacodeModel.MediaCodeDataSet);				
			}
			...
			{
				DataRow row = statisticsPgRegionDs.Media.Rows[i];
		...
		private void Init_cbSearchCategory()
		{
			...
			// 카테고리를 조회한다.
			StatisticsPgRegionModel statisticsPgRegionModel = new StatisticsPgRegionModel();

			statisticsPgRegionModel.Init();
			statisticsPgRegionModel.SearchMediaCode = MediaCode;

			new StatisticsPgRegionManager(systemModel, commonModel).GetCategoryList(statisticsPgRegionModel);
			
			if (statisticsPgRegionModel.ResultCD.Equals("0000"))
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgRegionDs.Category, statisticsPgRegionModel.CategoryDataSet);				
			}

			int CategoryCnt = statisticsPgRegionModel.ResultCnt;

			// 장르를 조회한다.
			statisticsPgRegionModel.Init();
			statisticsPgRegionModel.SearchMediaCode = MediaCode;

			new StatisticsPgRegionManager(systemModel, commonModel).GetGenreList(statisticsPgRegionModel);
			
			if (statisticsPgRegionModel.ResultCD.Equals("0000"))
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgRegionDs.Genre, statisticsPgRegionModel.GenreDataSet);				
			}
			...
			{
				DataRow row = statisticsPgRegionDs.Category.Rows[i];
		...
		private void SetcbSearchGenre(string CategoryCode)
		{
			...
			if(CategoryCode.Length > 0 && !CategoryCode.Equals("00"))
			{
				...
				DataRow[] genreRow =  statisticsPgRegionDs.Tables["Genre"].Select("CategoryCode = '" + CategoryCode +"'");
		...
		private void InitCombo_Level()
		{
			...
			{
				for(int i=0;i < statisticsPgRegionDs.Media.Rows.Count;i++)
				{
					DataRow row = statisticsPgRegionDs.Media.Rows[i];					
        ...
        {
            StatusMessage("지역별통계을 조회합니다.");

			if(cbSearchMedia.SelectedValue.Equals("00"))
			{
				MessageBox.Show("매체를 선택해 주십시오.", "지역별통계",
					MessageBoxButtons.OK, MessageBoxIcon.Information);
				return;
			}
			...
			try
			{
				...
				statisticsPgRegionModel.SearchMediaCode    =  cbSearchMedia.SelectedValue.ToString(); 
				statisticsPgRegionModel.SearchCategoryCode =  cbSearchCategory.SelectedValue.ToString(); 
				statisticsPgRegionModel.SearchGenreCode    =  cbSearchGenre.SelectedValue.ToString(); 
				statisticsPgRegionModel.SearchStartDay     =  cbSearchStartDay.Value.ToString("yyMMdd");
				statisticsPgRegionModel.SearchEndDay  	   =  cbSearchEndDay.Value.ToString("yyMMdd");

				// 데이터모델에 전송할 내용을 셋트한다.
				if(IsNewSearchKey)
				{
					statisticsPgRegionModel.SearchKey = "";
				}
				else
				{
					statisticsPgRegionModel.SearchKey  = ebSearchKey.Text;
				}
				...
				if(rbSearchType_C.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "C"; // -- C:계약기간
					IsAvg = false;
				}
				else if(rbSearchType_D.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "D"; // -- D:일간
					IsAvg = false;
				}
				else if(rbSearchType_W.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "W"; // -- W:주간
					IsAvg = false;
				}
				else if(rbSearchType_M.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "M"; // -- M:월간
					IsAvg = false;
				}
				else if(rbSearchType_1WA.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "1WA"; // --1WA:일주일평균
					IsAvg = true;
				}
				else if(rbSearchType_1MA.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "1MA"; // --1MA:1개월평균
					IsAvg = true;
				}
				else if(rbSearchType_2MA.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "2MA"; // --2MA:2개월평균
					IsAvg = true;
				}
				else if(rbSearchType_3MA.Checked)
				{
					statisticsPgRegionModel.SearchType	  	=  "3MA"; // --3MA:2개월평균
					IsAvg = true;
				}
				...
				if(IsAvg)
				{
					new StatisticsPgRegionManager(systemModel,commonModel).GetStatisticsPgRegionReportAVG(statisticsPgRegionModel);
				}
				else
				{
					new StatisticsPgRegionManager(systemModel,commonModel).GetStatisticsPgRegionReport(statisticsPgRegionModel);
				}

				if (statisticsPgRegionModel.ResultCD.Equals("0000"))
				{
					Utility.SetDataTable(statisticsPgRegionDs.ReportRegion, statisticsPgRegionModel.ReportDataSet);		
					StatusMessage(statisticsPgRegionModel.ResultCnt + "건이 조회되었습니다.");
					...
					keyReportType     = statisticsPgRegionModel.SearchType;


					uiPanelList.Text = "지역별통계 : " + keyMediaName;

					if(cbSearchCategory.SelectedItem.Value.ToString().Equals("00"))
					{
						uiPanelList.Text += " / 지역별 전체" ;
					}
					else
					{
			...
			{
				FrameSystem.showMsgForm("지역별통계 조회오류", new string[] {fe.ErrCode, fe.ResultMsg});
			}
			catch(Exception ex)
			{
				FrameSystem.showMsgForm("지역별통계 조회오류",new string[] {"",ex.Message});
			}
			finally
			{
				ProgressStop();
		...
		{	
			...
			{	
				...
				// 데이터 추출
				for (int inx =0; inx < statisticsPgRegionDs.ReportRegion.Rows.Count; inx++)
				{

					DataRow Row = statisticsPgRegionDs.ReportRegion.Rows[inx];			
					...
					{
						oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["SumName"].ToString();					// 1  대분류 지역명
					...
					{
						ColCnt++;	// 대분류 지역명은 건너뛴다
						oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["subSumName"].ToString();							// 2  소분류 지역명
					}
		...
		private void grdExReport_DrawGridArea(object sender, Janus.Windows.GridEX.DrawGridAreaEventArgs e)
		{
			if(e.Column.Key=="RateBar")
			{
				...
				if(e.Row.Cells["PgRate"].Value != null)
				{
					...
					rect.Width = (int)(rect.Width*(percentValue/100));  // 조회데이터가 늘 작으므로 10배 확대해서 보여준다. 100으로 나눌것을 10으로 나눔
