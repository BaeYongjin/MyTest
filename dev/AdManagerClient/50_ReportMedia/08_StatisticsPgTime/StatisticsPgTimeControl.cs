// ===============================================================================
//
// StatisticsPgTimeControl.cs
//
// 컨텐츠리포트 시간대별통계 컨트롤을 정의합니다. 
//
// ===============================================================================
// Release history
// ===============================================================================
...
{
    /// <summary>
    /// 컨텐츠리포트 시간대별통계 관리 컨트롤
    /// </summary>
    public class StatisticsPgTimeControl : System.Windows.Forms.UserControl, IUserControl
    {
        #region 이벤트핸들러
        ...
        // 사용할 정보모델
        StatisticsPgTimeModel statisticsPgTimeModel  = new StatisticsPgTimeModel();	// 프로그램별 광고시청집계모델
		...
		private AdManagerClient._50_ReportMedia.ReportMediaDs statisticsPgTimeDs;
        private System.ComponentModel.IContainer components;

        public StatisticsPgTimeControl()
        {
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(StatisticsPgTimeControl));
            ...
            this.statisticsPgTimeDs = new AdManagerClient._50_ReportMedia.ReportMediaDs();
            ...
            ((System.ComponentModel.ISupportInitialize)(this.statisticsPgTimeDs)).BeginInit();
            ...
            this.uiPanelChoiceAdSchedule.Text = "시간대별통계";
            // 
            // uiPanelSearch
            // 
            ...
            this.uiPanelSearch.Size = new System.Drawing.Size(1010, 86);
            ...
            this.uiPanelSearchContainer.Size = new System.Drawing.Size(1008, 84);
            ...
            this.pnlSearch.Size = new System.Drawing.Size(1008, 84);
            ...
            this.cbSearchEndDay.TabIndex = 8;
            ...
            this.uiPanelList.Location = new System.Drawing.Point(0, 112);
            this.uiPanelList.MinimumSize = new System.Drawing.Size(-1, 100);
            this.uiPanelList.Name = "uiPanelList";
            this.uiPanelList.Size = new System.Drawing.Size(1010, 565);
            ...
            this.uiPanelListContainer.Size = new System.Drawing.Size(1008, 541);
            ...
            this.grdExReport.Size = new System.Drawing.Size(1008, 541);
            this.grdExReport.TabIndex = 14;
            ...
            // dvReport
            // 
            this.dvReport.Table = this.statisticsPgTimeDs.ReportTime;
            // 
            // statisticsPgTimeDs
            // 
            this.statisticsPgTimeDs.DataSetName = "ReportMediaDs";
            this.statisticsPgTimeDs.Locale = new System.Globalization.CultureInfo("en-US");
            this.statisticsPgTimeDs.SchemaSerializationMode = System.Data.SchemaSerializationMode.IncludeSchema;
            // 
            // grdExRepoert
            // 
            ...
            ((System.ComponentModel.ISupportInitialize)(this.statisticsPgTimeDs)).EndInit();
		...
		private void Init_MediaCode()
		{
			...
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgTimeDs.Media, mediacodeModel.MediaCodeDataSet);				
			}
			...
			{
				DataRow row = statisticsPgTimeDs.Media.Rows[i];
		...
		private void Init_cbSearchCategory()
		{
			...
			// 카테고리를 조회한다.
			StatisticsPgTimeModel statisticsPgTimeModel = new StatisticsPgTimeModel();

			statisticsPgTimeModel.Init();
			statisticsPgTimeModel.SearchMediaCode = MediaCode;

			new StatisticsPgTimeManager(systemModel, commonModel).GetCategoryList(statisticsPgTimeModel);
			
			if (statisticsPgTimeModel.ResultCD.Equals("0000"))
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgTimeDs.Category, statisticsPgTimeModel.CategoryDataSet);				
			}

			int CategoryCnt = statisticsPgTimeModel.ResultCnt;

			// 장르를 조회한다.
			statisticsPgTimeModel.Init();
			statisticsPgTimeModel.SearchMediaCode = MediaCode;

			new StatisticsPgTimeManager(systemModel, commonModel).GetGenreList(statisticsPgTimeModel);
			
			if (statisticsPgTimeModel.ResultCD.Equals("0000"))
			{
				// 데이터셋에 셋팅
				Utility.SetDataTable(statisticsPgTimeDs.Genre, statisticsPgTimeModel.GenreDataSet);				
			}
			...
			{
				DataRow row = statisticsPgTimeDs.Category.Rows[i];
		...
		private void SetcbSearchGenre(string CategoryCode)
		{
			...
			if(CategoryCode.Length > 0 && !CategoryCode.Equals("00"))
			{
				...
				DataRow[] genreRow =  statisticsPgTimeDs.Tables["Genre"].Select("CategoryCode = '" + CategoryCode +"'");
		...
		private void InitCombo_Level()
		{
			...
			{
				for(int i=0;i < statisticsPgTimeDs.Media.Rows.Count;i++)
				{
					DataRow row = statisticsPgTimeDs.Media.Rows[i];					
        ...
        {
            StatusMessage("시간대별통계을 조회합니다.");

			if(cbSearchMedia.SelectedValue.Equals("00"))
			{
				MessageBox.Show("매체를 선택해 주십시오.", "시간대별통계",
					MessageBoxButtons.OK, MessageBoxIcon.Information);
				return;
			}
			...
			try
			{
				...
				statisticsPgTimeModel.SearchMediaCode    =  cbSearchMedia.SelectedValue.ToString(); 
				statisticsPgTimeModel.SearchCategoryCode =  cbSearchCategory.SelectedValue.ToString(); 
				statisticsPgTimeModel.SearchGenreCode    =  cbSearchGenre.SelectedValue.ToString(); 
				statisticsPgTimeModel.SearchStartDay     =  cbSearchStartDay.Value.ToString("yyMMdd");
				statisticsPgTimeModel.SearchEndDay  	   =  cbSearchEndDay.Value.ToString("yyMMdd");

				// 데이터모델에 전송할 내용을 셋트한다.
				if(IsNewSearchKey)
				{
					statisticsPgTimeModel.SearchKey = "";
				}
				else
				{
					statisticsPgTimeModel.SearchKey  = ebSearchKey.Text;
				}
				...
				if(rbSearchType_C.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "C"; // -- C:계약기간
					IsAvg = false;
				}
				else if(rbSearchType_D.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "D"; // -- D:일간
					IsAvg = false;
				}
				else if(rbSearchType_W.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "W"; // -- W:주간
					IsAvg = false;
				}
				else if(rbSearchType_M.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "M"; // -- M:월간
					IsAvg = false;
				}
				else if(rbSearchType_1WA.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "1WA"; // --1WA:일주일평균
					IsAvg = true;
				}
				else if(rbSearchType_1MA.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "1MA"; // --1MA:1개월평균
					IsAvg = true;
				}
				else if(rbSearchType_2MA.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "2MA"; // --2MA:2개월평균
					IsAvg = true;
				}
				else if(rbSearchType_3MA.Checked)
				{
					statisticsPgTimeModel.SearchType	  	=  "3MA"; // --3MA:2개월평균
					IsAvg = true;
				}
				...
				uiPanelList.Text = "시간대별통계"; 

				//  전체통계 서비스를 호출한다.
				if(IsAvg)
				{
					new StatisticsPgTimeManager(systemModel,commonModel).GetStatisticsPgTimeReportAVG(statisticsPgTimeModel);
				}
				else
				{
					new StatisticsPgTimeManager(systemModel,commonModel).GetStatisticsPgTimeReport(statisticsPgTimeModel);
				}

				if (statisticsPgTimeModel.ResultCD.Equals("0000"))
				{
					Utility.SetDataTable(statisticsPgTimeDs.ReportTime, statisticsPgTimeModel.ReportDataSet);		
					StatusMessage(statisticsPgTimeModel.ResultCnt + "건이 조회되었습니다.");
					...
					keyReportType     = statisticsPgTimeModel.SearchType;


					uiPanelList.Text = "시간대별통계 : " + keyMediaName;

					if(cbSearchCategory.SelectedItem.Value.ToString().Equals("00"))
					{
						uiPanelList.Text += " / 시간대별 전체" ;
					}
					else
					{
			...
			{
				FrameSystem.showMsgForm("시간대별통계 조회오류", new string[] {fe.ErrCode, fe.ResultMsg});
			}
			catch(Exception ex)
			{
				FrameSystem.showMsgForm("시간대별통계 조회오류",new string[] {"",ex.Message});
			}
			finally
			{
				ProgressStop();
		...
		{	
			...
			{	
				...
				// 헤더 정보 작성
				HeaderCount = 1;
				oSheet.Cells[HeaderRow,HeaderCount++] = "시간대"; 
				...
				// 데이터 추출
				for (int inx =0; inx < statisticsPgTimeDs.ReportTime.Rows.Count; inx++)
				{

					DataRow Row = statisticsPgTimeDs.ReportTime.Rows[inx];			

					int ColCnt = 1;
					
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["TimeName"].ToString();						// 1  시간대
