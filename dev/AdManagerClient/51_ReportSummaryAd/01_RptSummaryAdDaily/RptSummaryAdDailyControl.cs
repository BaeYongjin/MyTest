...
namespace AdManagerClient
{
    ...
    public class RptSummaryAdDailyControl : System.Windows.Forms.UserControl, IUserControl
	{
		...
		// 사용할 정보모델
		RptSummaryAdDailyModel rptSummaryAdDailyModel = new RptSummaryAdDailyModel();	// 광고일간레포트 모델
		...
		private void InitializeComponent()
		{
            ...
            this.uiPanelChoiceAdSchedule.Text = "일간 광고 집행실적";
            // 
            // uiPanelSearch
            // 
		...
		private void SearchReport()
		{
			...
			try
			{
				...
				if (rptSummaryAdDailyModel.ResultCD.Equals("0000"))
				{
					...
					StatusMessage(rptSummaryAdDailyModel.ResultCnt + "건이 조회되었습니다.");
		...
		{	
			...
			{	
				...
				oSheet.Cells[HeaderRow+1,HeaderCount++] = "시청횟수";
				...
				{		
					...
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["DayAdCnt"].ToString());				// 3  시청횟수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["DayAdUsers"].ToString());			// 4  시청가구
					...
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["AccuAdCnt"].ToString());			// 8  시청횟수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["AccuAdUsers"].ToString());			// 9  시청가구
					...
					if(N_ItemNo.Equals("99999999")) // 전체합계이면
					{
						oRng = oSheet.get_Range(GetColumnIndex(1)+Convert.ToString(DataRow+DataCount), GetColumnIndex(12)+Convert.ToString(DataRow+DataCount));
