...
namespace AdManagerClient
{
	...
	/// 광고별 시청현황 집계 관리 컨트롤
	/// </summary>
    public class AreaDayControl : System.Windows.Forms.UserControl, IUserControl
	{
		#region 이벤트핸들러
        ...
        private string          winName     = "지역-요일별 집계";
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(AreaDayControl));
            ...
            this.uiPanelChoiceAdSchedule.Text = "지역-요일별 집계";
            // 
            // uiPanelSearch
            // 
            ...
            this.ebContractName.TabIndex = 39;
            ...
            this.grdExReport.TotalRowFormatStyle.Appearance = Janus.Windows.GridEX.Appearance.Flat;
            ...
            // dvReport
            // 
            this.dvReport.Table = this.areaDayDs.AreaDay;
            // 
            // areaDayDs
            // 
        ...
        private void SearchReport()
        {
            ...
            try
            {
                ...
                //  전체통계 서비스를 호출한다.
                new AreaDayManager(systemModel,commonModel).GetAreaDay(areaDayModel);
                if (areaDayModel.ResultCD.Equals("0000"))
                {
                    Utility.SetDataTable(areaDayDs.AreaDay, areaDayModel.ReportDataSet);		
		...
		{	
			...
			{	
				...
				// 타이틀 작성
				oSheet.Cells[TitleRow,1] = "지역-요일별 집계";
				...
				DataCount = 0;
				// 데이터 추출
				for (int inx =0; inx < areaDayDs.AreaDay.Rows.Count; inx++)
				{		
					DataRow Row = areaDayDs.AreaDay.Rows[inx];

					int ColCnt = 1;

					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["RegionName"].ToString();                    
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["Mon"].ToString());			
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["Tue"].ToString());			
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToDecimal(Row["Wed"].ToString());
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["Thu"].ToString());
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt64(Row["Fri"].ToString());
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToDecimal(Row["Sat"].ToString());
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToDecimal(Row["Sun"].ToString());
