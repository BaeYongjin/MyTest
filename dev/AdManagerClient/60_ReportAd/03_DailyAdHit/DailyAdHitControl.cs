...
namespace AdManagerClient
{
    ...
    /// 광고 일별시청률 관리 컨트롤
    /// </summary>
    public class DailyAdHitControl : System.Windows.Forms.UserControl, IUserControl
    {
      ...
      private MenuPower     menu          = FrameSystem.oMenu;
        ...
        // 사용할 정보모델
        DailyAdHitModel mainModel  = new DailyAdHitModel();	// 프로그램별 광고시청집계모델
		...
		private AdManagerClient.DailyAdHitDs dailyAdHitDs;
        private AdManagerClient.ReportHeaderControl rptHeader;
        private System.ComponentModel.IContainer components;

        public DailyAdHitControl()
        {
        ...
        private void InitializeComponent()
        {
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(DailyAdHitControl));
			...
			this.dailyAdHitDs = new AdManagerClient.DailyAdHitDs();
			...
			((System.ComponentModel.ISupportInitialize)(this.dailyAdHitDs)).BeginInit();
			...
			this.uiPanelChoiceAdSchedule.Text = "일별 광고시청 집계";
			// 
			// uiPanelSearch
			// 
			...
			this.uiPanelList.Text = "일별 장르별 광고노출 현황";
			// 
			...
			this.grdExHitList.DataSource = this.dailyAdHitDs.Report;
			...
			this.grdExHitList.Size = new System.Drawing.Size(1008, 516);
			...
			// dailyAdHitDs
			// 
			this.dailyAdHitDs.DataSetName = "DailyAdHitDs";
			this.dailyAdHitDs.Locale = new System.Globalization.CultureInfo("en-US");
			this.dailyAdHitDs.SchemaSerializationMode = System.Data.SchemaSerializationMode.IncludeSchema;
			// 
			// panel3
			// 
			...
			((System.ComponentModel.ISupportInitialize)(this.dailyAdHitDs)).EndInit();
		...
		private void SetListData()
		{
			...
			dailyAdHitDs.Report.Clear();		// 데이터 클리어

			DataRow reportRow = dailyAdHitDs.Report.NewRow();

			for(int i=0;i<dailyAdHitDs.RowData.Rows.Count;i++)
			{
				DataRow dataRow = dailyAdHitDs.RowData.Rows[i];
				...
				if(!OldDay.Equals(LogDay))
				{
					if(!OldDay.Equals(""))
					{
						...
						dailyAdHitDs.Report.Rows.Add(reportRow); // 리포트데이터에 Row셋트
					}

					reportRow = dailyAdHitDs.Report.NewRow();
					OldDay = LogDay;
		...
		private void SetListHeader()
		{
			...
			for(int i=0;i<dailyAdHitDs.Header.Rows.Count;i++)
			{
				DataRow Row = dailyAdHitDs.Header.Rows[i];
...
//			FrameSystem.oLog.Debug(LayoutXml.ToString());
		...
		{	
			...
			{	
				...
				// 헤더의 장르의 갯수
				GenreCount = dailyAdHitDs.Header.Rows.Count;
				...
				// 타이틀 작성
				oSheet.Cells[TitleRow,1] = "일별 광고시청 집계";
				...
				// TODO : 2014-10-22 Excel 0x800A03EC오류 문제
				for(int i=0;i<GenreCount;i++)
				{
					DataRow Row = dailyAdHitDs.Header.Rows[i];				// 헤더데이터로부터 장르를 헤더로 셋트한다.
					...
					if(!OldCategory.Equals(NowCategory))
					{
						...
						{ 
							...
							oSheet.Cells[HeaderRow, GetColumnIndex(bgncol)] = OldCategory;
							bgncol += colcnt;
							colcnt  = 0;
						}
					}
					...
					oSheet.Cells[HeaderRow + 1, GetColumnIndex(3 + i)] = Row["GenreName"].ToString(); //장르명 . 처음 장르가 시작되는 컬럼위치 3번째부터
				...
				DataCount = 1;
				// 데이터 추출
				for (int inx =0; inx < dailyAdHitDs.Report.Rows.Count; inx++)
				{

					oSheet.Cells[DataRow+DataCount,1] = dailyAdHitDs.Report.Rows[inx]["LogDay"].ToString();
					int SumDay = Convert.ToInt32(dailyAdHitDs.Report.Rows[inx]["SumDay"].ToString());	
					...
					for(int i=0;i<GenreCount;i++)
					{
						...
						int Hit = Convert.ToInt32(dailyAdHitDs.Report.Rows[inx]["Genre"+i.ToString()].ToString());
						//oSheet.Cells[DataRow + DataCount, 3 + i] = Hit;
						oSheet.Cells[DataRow + DataCount, GetColumnIndex(3 + i)] = Hit;
						TotalHit[i] += Hit;
					}					
					DataCount++;
				}
				...
				for(int i=0;i<GenreCount;i++)
				{
					...
					oSheet.Cells[DataRow, GetColumnIndex(3 + i)] = TotalHit[i];
				}	
        ...
        private void OnSearch(object sender, SearchReportData e)
        {
            ...
            try
            {
                ...
                uiPanelList.Text = "일별 광고시청"; 
			 
                //  전체통계 서비스를 호출한다.
                new DailyAdHitManager(systemModel,commonModel).GetDailyAdHitReport(mainModel);
	
                if (mainModel.ResultCD.Equals("0000"))
                {
                    Utility.SetDataTable(dailyAdHitDs.RowData, mainModel.ReportDataSet);		
                    Utility.SetDataTable(dailyAdHitDs.Header,  mainModel.HeaderDataSet);		
            ...
            {
                rptHeader.u_IsPrint = false;
                FrameSystem.showMsgForm("일별 광고시청 조회오류", new string[] {fe.ErrCode, fe.ResultMsg});
            }
            catch(Exception ex)
            {
                rptHeader.u_IsPrint = false;
                FrameSystem.showMsgForm("일별 광고시청 조회오류",new string[] {"",ex.Message});
            }
            finally
            {
                ProgressStop();
