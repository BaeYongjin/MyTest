...
namespace AdManagerClient
{
    ...
    public class AdHitStatusControl : System.Windows.Forms.UserControl, IUserControl
    {
        ...
        // 사용할 정보모델
        AdHitStatusModel adHitStatusModel  = new AdHitStatusModel();	// 프로그램별 광고시청집계모델
        ...
        private void InitializeComponent()
        {
			...
			this.uiPanelChoiceAdSchedule.Text = "광고별 시청현황";
			// 
			// uiPanelSearch
			// 
      ...
      {
			StatusMessage("광고별 시청현황을 조회합니다.");

			if(cbSearchMedia.SelectedValue.Equals("00"))
			{
				MessageBox.Show("매체를 선택해 주십시오.", "광고별 시청현황",MessageBoxButtons.OK, MessageBoxIcon.Information);
				return;
			}
			...
			try
			{
				...
				if (adHitStatusModel.ResultCD.Equals("0000"))
				{
					...
					StatusMessage(adHitStatusModel.ResultCnt + "건이 조회되었습니다.");
				
					keyMediaName      = cbSearchMedia.SelectedItem.Text;
					keyReportDay   = cbSearchDay.Value.ToString("yyyy-MM-dd");

					uiPanelList.Text = "광고별 시청현황 : " + keyMediaName + " / " + keyReportDay;
			...
			{
				FrameSystem.showMsgForm("광고별 시청현황 조회오류", new string[] {fe.ErrCode, fe.ResultMsg});
			}
			catch(Exception ex)
			{
				FrameSystem.showMsgForm("광고별 시청현황 조회오류",new string[] {"",ex.Message});
			}
			finally
			{
				ProgressStop();
		...
		{	
			...
			{	
				...
				// 타이틀 작성
				oSheet.Cells[TitleRow,1] = "광고별 시청현황";
				...
				oSheet.Cells[HeaderRow,HeaderCount++] = "당일시청수";
				oSheet.Cells[HeaderRow,HeaderCount++] = "누적시청수";
				oSheet.Cells[HeaderRow,HeaderCount++] = "예상시청수(계약)";
				oSheet.Cells[HeaderRow,HeaderCount++] = "예상시청수(실제)";
				oSheet.Cells[HeaderRow,HeaderCount++] = "예상시청수(당일)";
				oSheet.Cells[HeaderRow,HeaderCount++] = "계약(보장노출)수량";
				oSheet.Cells[HeaderRow,HeaderCount++] = "집행시작일";
				oSheet.Cells[HeaderRow,HeaderCount++] = "집행종료일";
				oSheet.Cells[HeaderRow,HeaderCount++] = "실제종료일";
				oSheet.Cells[HeaderRow,HeaderCount++] = "노출일수";
				oSheet.Cells[HeaderRow,HeaderCount++] = "계약일수";
				oSheet.Cells[HeaderRow,HeaderCount++] = "실제일수";
				oSheet.Cells[HeaderRow,HeaderCount++] = "광고계약명";
				oSheet.Cells[HeaderRow,HeaderCount++] = "대행사";
				...
				for (int inx =0; inx < adHitStatusDs.Report.Rows.Count; inx++)
				{
					...
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["AdCnt"].ToString());		// 3  당일시청수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["TotAdCnt"].ToString());	// 4  누적시청수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["ExpCntExcu"].ToString());	// 5  예상시청수(계약)
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["ExpCntReal"].ToString());	// 6  예상시청수(실제)
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["ExpCntDay"].ToString());	// 7  예상시청수(당일)
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["ContractAmt"].ToString());	// 8  계약시청수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Utility.reConvertDate(Row["ExcuteStartDay"].ToString());		// 9  집행시작일
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Utility.reConvertDate(Row["ExcuteEndDay"].ToString());		// 10  집행종료일
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Utility.reConvertDate(Row["RealEndDay"].ToString());			// 11 실제종료일
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["ExDays"].ToString());		// 12 노출일수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["TotDaysExcu"].ToString());	// 13 계약일수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["TotDaysReal"].ToString());	// 14 실제일수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["ContractName"].ToString();					// 15 광고계약명
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["AgencyName"].ToString();					// 16 대행사
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["AdvertiserName"].ToString();				// 17 광고주
				...
				// 집행시작일 외 중앙정렬
				oRng = oSheet.get_Range(GetColumnIndex(9)+Convert.ToString(DataRow), GetColumnIndex(11)+Convert.ToString(DataRow+DataCount)); // 일자
				oRng.VerticalAlignment   = Excel.XlVAlign.xlVAlignCenter;	// 세로중앙정렬
				oRng.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;	// 가로중앙정렬	 
				...
				// 당일시청수 외 데이터에 셀타입 설정
				oRng = oSheet.get_Range(GetColumnIndex(3)+Convert.ToString(DataRow), GetColumnIndex(8)+Convert.ToString(DataRow+DataCount));	// 데이터의 범위
				oRng.NumberFormatLocal = "#,##0";

				// 노출일수 외 데이터에 셀타입 설정
				oRng = oSheet.get_Range(GetColumnIndex(12)+Convert.ToString(DataRow), GetColumnIndex(14)+Convert.ToString(DataRow+DataCount));	// 데이터의 범위
				oRng.NumberFormatLocal = "#,##0";
