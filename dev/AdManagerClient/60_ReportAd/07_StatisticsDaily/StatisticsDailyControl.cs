...
namespace AdManagerClient
{
    ...
    /// 광고별 시청현황 집계 관리 컨트롤
    /// </summary>
    public class StatisticsDailyControl : System.Windows.Forms.UserControl, IUserControl
    {
        #region 이벤트핸들러
      ...
      // 사용할 정보모델
		StatisticsDailyModel statisticsDailyModel  = new StatisticsDailyModel();	// 프로그램별 광고시청집계모델
		...
		private AdManagerClient.StatisticsDailyDs statisticsDailyDs;
        ...
        private void InitializeComponent()
        {
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(StatisticsDailyControl));
			...
			this.statisticsDailyDs = new AdManagerClient.StatisticsDailyDs();
			...
			((System.ComponentModel.ISupportInitialize)(this.statisticsDailyDs)).BeginInit();
			...
			this.uiPanelChoiceAdSchedule.Text = "일별통계";
			// 
			// uiPanelSearch
			// 
			...
			this.ebContractAmt.TabIndex = 183;
			...
			this.ebTotalAdCnt.TabIndex = 183;
			...
			this.grdExReport.TotalRow = Janus.Windows.GridEX.InheritableBoolean.True;
			...
			// dvReport
			// 
			this.dvReport.Table = this.statisticsDailyDs.Report;
			// 
			// statisticsDailyDs
			// 
			this.statisticsDailyDs.DataSetName = "StatisticsDailyDs";
			this.statisticsDailyDs.Locale = new System.Globalization.CultureInfo("en-US");
			this.statisticsDailyDs.SchemaSerializationMode = System.Data.SchemaSerializationMode.IncludeSchema;
			// 
			// grdExRepoert
			// 
			...
			((System.ComponentModel.ISupportInitialize)(this.statisticsDailyDs)).EndInit();
		...
		{	
			...
			{	
				...
				// 타이틀 작성
				oSheet.Cells[TitleRow,1] = "일별 광고 통계";
				...
				oRng = oSheet.get_Range("B"+Convert.ToString(ConditionRow+CondCount), TitleCol+Convert.ToString(ConditionRow+CondCount));
				oRng.Merge(true);
				//if(keyItemNo.Equals("00") || keyItemNo.Equals(""))
				//{
				//	oSheet.Cells[ConditionRow+CondCount,2] = "전체광고";
				//}
				//else
				//{
				...
				DataCount = 0;
				// 데이터 추출
				for (int inx =0; inx < statisticsDailyDs.Report.Rows.Count; inx++)
				{

					DataRow Row = statisticsDailyDs.Report.Rows[inx];			

					int ColCnt = 1;
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Row["LogDay"].ToString();						// 1  일자
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToInt32(Row["AdCnt"].ToString());		// 2  노출수
					oSheet.Cells[DataRow+DataCount,ColCnt++] = Convert.ToDecimal(Row["AdRate"].ToString());		// 3  %
				...
				// 일자 중앙정렬
				oRng = oSheet.get_Range(GetColumnIndex(1)+Convert.ToString(DataRow), GetColumnIndex(1)+Convert.ToString(DataRow+DataCount)); // 일자
				oRng.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;	// 가로중앙정렬	 
		...
		private void grdExReport_DrawGridArea(object sender, Janus.Windows.GridEX.DrawGridAreaEventArgs e)
		{
		 ...
		 SearchReportData	_SearchData;
		 /// <summary>
		 /// 사용자컨트롤에서 조회버튼을 클릭이벤트 발생처리
		 /// </summary>
		 /// <param name="sender"></param>
		 /// <param name="e"></param>
		private void OnSearch(object sender, SearchReportData e)
		{
			...
			{
				// 데이터모델 초기화
			 	statisticsDailyModel.Init();
			 
			 	statisticsDailyModel.SearchMediaCode   =  e.MediaCode;
			 	statisticsDailyModel.SearchContractSeq =  e.ContractSeq;
				statisticsDailyModel.CampaignCode		=  e.CampaignNo;
			 	statisticsDailyModel.SearchItemNo      =  e.ItemNo;
			 	statisticsDailyModel.SearchStartDay    =  e.ItemBeginDay;
			 	statisticsDailyModel.SearchEndDay  		=  e.ItemEndDay;
			 	...
			 	//  전체통계 서비스를 호출한다.
			 	new StatisticsDailyManager(systemModel,commonModel).GetStatisticsDailyReport(statisticsDailyModel);
	
				if (statisticsDailyModel.ResultCD.Equals("0000"))
				{
					Utility.SetDataTable(statisticsDailyDs.Report, statisticsDailyModel.ReportDataSet);		
					StatusMessage(statisticsDailyModel.ResultCnt + "건이 조회되었습니다.");
			 	
					keyTotalAdCnt     = statisticsDailyModel.TotalAdCnt.ToString();
					keyContractAmt    = statisticsDailyModel.ContractAmt.ToString();
